...
function connectionScope(connectionId?: string): string {
  ...
  if (!/^[A-Za-z0-9_-]{12,128}$/.test(connectionId)) throw new LivePilotError('INVALID_STATE', 'OAuth Connection ID 无效。', { retryable: false })
  return connectionId
}

/** Returns the encrypted token path for one server-issued connection ID. */
function tokenPath(connectionId?: string): string {
  const scope = connectionScope(connectionId)
  return scope === 'legacy' ? dataPath('youtube-tokens.enc') : dataPath('connections', scope, 'youtube-tokens.enc')
}
