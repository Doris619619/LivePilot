...
const DEFAULT_ACTIONS: Record<PublicErrorCode, string> = {
  ...
  RUN_ALREADY_ACTIVE: '该 YouTube Channel 已有正在运行或需要恢复的 Live Run。',
  MEDIA_NOT_FOUND: '重新扫描媒体目录后选择仍然存在的文件。',
  MEDIA_INVALID: '使用受支持且通过预检的本地视频或 MP3 音乐。',
  FFMPEG_UNAVAILABLE: '安装随应用提供的 FFmpeg，或配置受信任的 LIVEPILOT_FFMPEG_PATH。',
  WORKER_START_FAILED: '检查媒体文件、FFmpeg 安装和本机权限后重试。',
  WORKER_CRASHED: 'FFmpeg 已异常退出；查看已脱敏的运行状态并重新准备直播。',
  WORKER_UNRESPONSIVE: 'FFmpeg 未报告新的进度；检查本机资源和媒体文件后结束或重试。',
  INGEST_TIMEOUT: 'FFmpeg 已启动但 YouTube 未确认 ingest active；检查网络和频道直播权限。',
