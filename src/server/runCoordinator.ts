...
export class RunCoordinator {
  ...
  async refreshObs(channelId: string): Promise<void> {
    const obs = await requireObsForChannel(channelId); const client = new ObsControlClient()
  ...
  async start(channelId: string): Promise<void> {
    ...
    await withChannelLock(channel.connectionId, channel.id, 'start-run', async () => {
      const obs = await requireObsForChannel(channel.id); const client = new ObsControlClient()
      let initial
      ...
      const run = await createRun(channel.id, obs.id); const api = createChannelYouTubeApi(channel.connectionId); let obsStarted = false
      try {
        ...
        let ingest = null
        for (let attempt = 0; attempt < POLL_ATTEMPTS; attempt += 1) { ingest = await api.getStreamStatus(stream.streamId); await updateRun(run.id, { ingestStatus: ingest.streamStatus }); if (ingest.streamStatus === 'active') break; if (attempt + 1 < POLL_ATTEMPTS) await sleep(POLL_MS) }
        ...
        for (let attempt = 0; attempt < POLL_ATTEMPTS; attempt += 1) { lifecycle = await api.getBroadcastLifeCycleStatus(broadcast.id); if (lifecycle === 'live') break; if (attempt + 1 < POLL_ATTEMPTS) await sleep(POLL_MS) }
  ...
  async stop(runId: string): Promise<void> {
    ...
    await withChannelLock(channel.connectionId, channel.id, 'stop-run', async () => {
      ...
      let lifecycle: string | null = null
      for (let attempt = 0; attempt < POLL_ATTEMPTS; attempt += 1) { lifecycle = await api.getBroadcastLifeCycleStatus(latest.broadcastId); if (lifecycle === 'complete') break; if (attempt + 1 < POLL_ATTEMPTS) await sleep(POLL_MS) }
